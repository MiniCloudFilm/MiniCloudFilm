  ...
  getDoctor: function() {
    wx.request({
      ...
      success: res => {
        ...
        for (let i = 0; i < this.data.department; i++) {
          // if (this.data.doctorList.deptId == this.data.department[i].deptId) { }
        }
        if (doctor.areaId) {
          this.getArea(doctor.cityId, 3) 
          // for (let i = 0; i < department.length;i++){
          //   if (department[i].areaId==doctor.areaId){
